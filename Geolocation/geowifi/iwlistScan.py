...
def get_frequency(cell):
    frequency =  matching_line(cell,"Frequency:").split(" ")[0]
...
def get_distance(cell):
    freq = float (get_frequency(cell).split(" ")[0])
    freqInMHz = freq*1000
    db = float (get_signal_level(cell).split(" ")[0])
    return str(round(calculateDistanceInM(db , freqInMHz),2)) + " m"


def calculateDistanceInM(levelInDb, freqInMHz):
...
columns=["Name","Address","Quality","Signal", "Channel","Frequency","Distance" ,"Encryption"]
